...
const generatorEventList = (size=3) => {
  const eventList = Array(size).fill().map((_,index)=>{
    return (
      {id:index+1, name:faker.git.commitMessage(), location: faker.address.streetName(), hostname:faker.name.firstName(), type:faker.random.boolean()?"Private":"Public", date:faker.date.between('2018-01-01', '2022-01-05')}
    );
  });
...
class App extends Component {
  ...
  setStateSelectAll = (val)=>{
    ...
    if(val){
      ...
      let lastDay = new Date(this.state.yearLowerLimit+this.state.yearAmmountLimit, 11, 31);
  ...
  setStateDateFromQuarter = (month1, month2) => {
    ...
    /*if(month1<=this.state.month1 || month2===this.state.month2){
